...
function feed(args) { 
    if(!Array.isArray(args) || args.length === 0) { 
        console.log('please pass in a set of words')
        return
    }
    ...
    function traverse(current, array) {
        if(array.length === 0) { 
            return
        }
        
        if(current[array[0]]) { 
            current[array[0]][hashKey].hitCount += 1 
        } else { 
            current[array[0]] = { 
                [hashKey]: {
                    hitCount: 0, 
                    description: "llm-feed-value", 
                    file: "", 
                    executable: false, 
                    responses: [], 
                    nextServer: false 
        ...
        copy.shift()
        return traverse(current[array[0]], copy)
        
    }
    
    traverse(categoricalNoteTree, args)
    writeToFile('categoricalNoteTree.json', categoricalNoteTree)
...
function responses(args) { 
    if(!Array.isArray(args) || args.length < 3) { 
        console.log('input must contain at least two arguments <responseCount = number> <fullTraversal = string> <path = words>')
        console.log('ex all empty responses starting at root: ctllmResponse 0 all')
        return
    }

    if(typeof(parseInt(args[0])) !== 'number') { 
        console.log('please pass in a number as the first argument')
        console.log('the number is the amount of responses a specific-path has, responses returned')
        return
    }

    if(args[1] !== 'all' && args[1] !== 'one') { 
        console.log('the second argument must be the word "one" or "all"')
        console.log('the second argument determines whether to traverse the entire directory')
        return
    }

    let responseCount = parseInt(args[0])
    let traverseAll = args[1]
    let path = args.slice(2, args.length)
    let categoricalNoteTree = readFromFile('categoricalNoteTree')
    let current = referenceObject(categoricalNoteTree, 0, path)
    ...
    function traverse(curr) { 
        for(const [key, value] of Object.entries(curr)) { 
            if(key !== hashKey) { 
                keyPath.push(key)
                traverse(curr[key])
                let hashObject = curr[key][hashKey]
                if(hashObject.responses.length === responseCount) { 
                    pathResponses.push({ 
                        path: [...path, ...keyPath],
                        responses: hashObject.responses
                    })
                }
                keyPath.pop(key)
            }
